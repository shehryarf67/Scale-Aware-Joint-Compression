...
\textbf{not supported}. The observed direction is the opposite, at both budgets. Three
considerations prevent us from stating this as a refutation:
...
have, while its end-to-end benefit collapses. If the headroom explanation is correct, joint
gain should track compression severity rather than scale, and would reappear at 1B under a
budget aggressive enough to degrade it comparably to 160M. If the depth explanation is
correct, gain should track block count, and a deeper model at the same parameter count would
show more. These make different predictions and could be separated by a depth-controlled
experiment; we did not run one, and we flag the confound rather than adjudicating it from
data that cannot.
